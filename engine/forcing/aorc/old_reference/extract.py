...
def main():
    ...
    # subset spatially to only the unique pixels actually used — avoids
    # downloading a bounding-box rectangle, which can be >100x larger than
    # the pixels used when catchments are geographically scattered (e.g.
    # several disjoint HUC8s across CONUS)
    print("  Subsetting to unique pixels used...")
    if weighted:
        flat_ids = np.concatenate(cell_ids_list)
        uniq_ids, inverse = np.unique(flat_ids, return_inverse=True)
        splits = np.cumsum([len(c) for c in cell_ids_list])[:-1]
        local_cids_list = np.split(inverse, splits)
    else:
        flat_ids = np.asarray(row_flat, dtype=np.int64) * AORC_NCOLS + np.asarray(
            col_flat, dtype=np.int64,
        )
        uniq_ids, local_pixel_idx = np.unique(flat_ids, return_inverse=True)

    ds = spatial_subset_points(ds, uniq_ids)

    if do_15min:
        print("  Will disaggregate to 15-min after extraction")

    # compute catchment centroids from the selected-pixel coords (used in NetCDF output)
    lat_bbox = ds.latitude.values
    lon_bbox = ds.longitude.values

    if weighted:
        cat_lats, cat_lons = [], []
        for cids, w in zip(local_cids_list, weights_list):
            wn = w / w.sum()
            cat_lats.append(float(np.dot(wn, lat_bbox[cids])))
            cat_lons.append(float(np.dot(wn, lon_bbox[cids])))
    else:
        cuts = np.cumsum(pixel_counts)[:-1]
        idx_per = np.split(local_pixel_idx, cuts)
        cat_lats = [float(lat_bbox[idx].mean()) for idx in idx_per]
        cat_lons = [float(lon_bbox[idx].mean()) for idx in idx_per]
    ...
    if weighted:
        n_pixels = len(lat_bbox)
        print("  Building sparse weight matrix...")
        W = build_weight_matrix(local_cids_list, weights_list, n_basins, n_pixels)
    ...
    for var_name in args.variables:
        ...
        raw = ds_computed[var_name].values  # (hours, n_unique_pixels)

        if weighted:
            result = weighted_mean(raw, W)
        else:
            raw_sel = raw[:, local_pixel_idx]  # (hours, total_pixels, w/ dupes)
