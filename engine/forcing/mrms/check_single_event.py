...
OUT_DIR = Path('/projects/mhpi/leoglonz/sub_hourly/data/_mrms_preprocess/event_checks') / f'event_{EVENT_ID}'
...
def main() -> None:
    ...
    print(f'{len(divide_ids)} catchments with data in existing file '
    ...
    print(f'{len(times)} timestamps to fetch (native 2-min MRMS cadence, '
    ...
    # ---- load the freshly downloaded shards, extract nearest-cell precip
    # rate per catchment centroid, resample to 15-min depth, same as the
    # pipeline (Notebook 4: storm_catchment_rate_2min + to_depth_15) ----
    ...
    rate2min = pd.DataFrame(pts.values, index=pd.DatetimeIndex(da['time'].values), columns=cat_ids)
    ...
    print('basin-mean total depth over event (mm):', np.nanmean(np.nansum(depth15.values, axis=0)))
    print('n timesteps with any nonzero rain (any catchment):',
          int((np.nansum(depth15.values, axis=1) > 0.01).sum()), '/', depth15.shape[0])
