"""MRMS x HydroFabric pipeline: CONUS cell/catchment crosswalk, per-event
upstream catchments, raw MRMS PrecipRate download, and area-weighted 2-min ->
15-min catchment precipitation extraction. Orchestrated by
engine/forcing/mrms/run_pipeline.py; this module holds the library logic.

Decoder note: PrecipRate grib2s are decoded via the low-level eccodes API
directly (codes_new_from_message + codes_get_values) rather than cfgrib's
xr.open_dataset(engine="cfgrib"), which does much more work than a single-
variable single-message file needs (~12 files/s vs ~2 files/s). Falls back to
cfgrib if the fast path raises. Sources: AWS noaa-mrms-pds (anon S3) with
Iowa State HTTPS fallback.
"""
...
WINDOW_DAYS = 5.0  # default event forcing-window width (days); override via build_manifest(window_days=...)
...
def build_manifest(events: pd.DataFrame, cache_dir: Path, tag: str = "",
                    window_days: float = WINDOW_DAYS, centroid: str = "midpoint"):
    ...
    if centroid == "peak":
        ...
        ev["centroid_time"] = pd.to_datetime(ev["peak_time"], errors="coerce", utc=True).dt.tz_localize(None)
    else:
        ev["centroid_time"] = ev["begin_time"] + (ev["end_time"] - ev["begin_time"]) / 2

    # Round outward to the HOUR (a strict subset of :00/:15/:30/:45, so this
    # still satisfies "on the 15-min grid") rather than the nearest 15-min
    # mark. This must exactly match aorc/to_events.py's own window boundary
    # (`ant_end = (centroid - ...).astype('datetime64[h]')`), which the
    # merge step (merge_15min.py) relies on implicitly: it copies P and T/PET
    # into the combined file by raw row position with no timestamp
    # cross-check, assuming index 0 in both sources is the same real-world
    # time. Flooring/ceiling to 15-min instead of the hour here would silently
    # offset MRMS's window from AORC's by up to 45 minutes whenever the
    # centroid isn't already hour-aligned, misaligning precipitation against
    # temperature/PET in the merged forcing by up to three 15-min steps.
...
def _decode_precip_eccodes(raw_gz: bytes) -> xr.DataArray:
    ...
    return xr.DataArray(values, dims=("latitude", "longitude"),
                         coords={"latitude": lat, "longitude": lon}, name="precip_rate")
...
def _write_day(path, existing_da, new_slices):
    ...
    da_all = da_all.assign_coords(time=pd.DatetimeIndex(pd.to_datetime(da_all["time"].values)))
...
def _run_pass(days, todo_by_day, bbox, fs, mask_negative, pool, bar, shard_dir, max_workers, session=None):
    """Fetch every (day, timestamp) in todo_by_day using ONE rolling
    submission window across ALL days, not one day at a time -- keeps the
    pool continuously saturated across day boundaries instead of blocking new
    submissions on `as_completed()` for the current day plus that day's
    synchronous _write_day() before starting the next. A day's file is
    written as soon as its last outstanding timestamp resolves, without
    stalling fetches for any other day.
    ...
    day_remaining = {d: len(todo_by_day.get(d) or []) for d in days if todo_by_day.get(d)}
    day_stats = {d: dict(new=0, missing=0, errors=0) for d in day_remaining}
    new_missing: list = []
    ...
    def _flush_day(d):
        ...
        stats = day_stats[d]
        results[d] = dict(date=f"{d:%Y-%m-%d}", stored=n_stored, new=stats["new"],
...
def build_store(unique_times, bbox, out_dir, max_workers=12,
    ...
    """Download + bbox-subset every timestamp, writing one NetCDF per UTC day
    under <out_dir>/shards/. Resumable: rerun to fill gaps; confirmed-missing
    timestamps (404 in both archives) are remembered and skipped next time.
    Transient failures (timeouts, decode errors -- not a confirmed absence)
    are retried in-process up to max_retries times; any still failing after
    that are logged to transient_errors.csv and retried again next call."""
    ...
    if use_aws and s3fs is not None:
        try:
            # Default botocore connection pool (10) is far smaller than
            # max_workers can be; with more worker threads than pooled
            # connections, requests queue on the pool itself regardless of
            # thread count, capping throughput well below what max_workers
            # implies. Size it to match.
    ...
    # Pooled, keep-alive session for the Iowa State fallback -- without this,
    # every fallback request opens a brand-new TCP+TLS connection via urllib,
    # pure overhead under high concurrency. Sized to match max_workers so
    # threads don't queue on the pool itself (same reasoning as the s3fs
    # config_kwargs above).
    ...
    try:
        results, new_missing, fail_by_day = _run_pass(
            days, todo_by_day, bbox, fs, mask_negative, pool, bar, shard_dir, max_workers, session)
        ...
        while fail_by_day and retry_round < max_retries:
            ...
            retry_bar = tqdm(total=n_retry, desc=f"retry {retry_round}", unit="file",
                              disable=not verbose) if bar is not None else None
    ...
    summary = dict(
        days=len(days), to_fetch=int(total_todo),
        stored_new=int(sum(r["new"] for r in results.values())),
        confirmed_missing=int(len(new_missing)), transient_errors=int(len(persistent_fails)),
        shard_dir=shard_dir, manifest=manifest_csv,
    )
    if verbose:
        print("Done:", summary)
...
def storm_catchment_rate_2min(ds: xr.Dataset, win_start, win_end, divide_ids, frac_cw: pd.DataFrame,
    ...
    """Returns (rate_df, reason). rate_df is None (with a `reason` string) if
    the window has no data at all, no crosswalk match, or -- critically --
    incomplete day coverage: if build_store() failed to fetch every single
    timestamp for a given UTC day, that day never gets a shard file at all
    (see _write_day), so `.sel(time=slice(...))` below would otherwise
    silently compute a "valid" depth from whatever partial days ARE present.
    That can turn a real multi-day storm into an apparent all-zero event
    (observed for event 1891: 2 of 6 needed days present, both dry, silently
    written as 0.0 mm instead of the ~93mm the other 4 days actually had).
    Below-threshold coverage is treated as a hard failure instead, so it
    surfaces in extraction_failed_events.csv rather than corrupting P."""
    ...
    if actual_n < min_coverage * expected_n:
        ...
        all_days = set(pd.date_range(win_start.normalize(), win_end.normalize(), freq="D"))
        missing_days = sorted(d.date() for d in (all_days - present_days))
        return None, (f"incomplete window coverage ({actual_n}/{expected_n} timestamps, "
    ...
    # over non-NaN cells only (equivalent to a per-timestep groupby, but ~2
    # matmuls instead of a pandas groupby over time*cell rows).
...
def extract_all(manifest: pd.DataFrame, event_catchment_windows: pd.DataFrame,
    ...
    for _, row in tqdm(manifest.iterrows(), total=len(manifest), desc="extract"):
        ...
        if not divide_ids:
            ...
            continue
        depth15, reason = storm_catchment_rate_2min(ds, row["win_start"], row["win_end"], divide_ids, frac_cw)
...
def merge_parts(part_paths, out_nc):
    ...
    all_cats = sorted(set().union(*[set(p["divide_id"].values.tolist()) for p in parts]))
    cat_idx = {c: j for j, c in enumerate(all_cats)}
